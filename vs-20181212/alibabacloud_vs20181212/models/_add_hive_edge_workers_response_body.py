...
class AddHiveEdgeWorkersResponseBody(DaraModel):
    def __init__(
        self,
        failed_instance_count: int = None,
        failed_instances: List[main_models.AddHiveEdgeWorkersResponseBodyFailedInstances] = None,
        request_id: str = None,
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('FailedInstances') is not None:
            for k1 in m.get('FailedInstances'):
                temp_model = main_models.AddHiveEdgeWorkersResponseBodyFailedInstances()
        ...
        if m.get('SuccessInstances') is not None:
            for k1 in m.get('SuccessInstances'):
                temp_model = main_models.AddHiveEdgeWorkersResponseBodySuccessInstances()
                self.success_instances.append(temp_model.from_map(k1))

        return self

class AddHiveEdgeWorkersResponseBodySuccessInstances(DaraModel):
    def __init__(
        self,
...
class AddHiveEdgeWorkersResponseBodyFailedInstances(DaraModel):
    def __init__(
        self,
        instance_id: str = None,
        message: str = None,
