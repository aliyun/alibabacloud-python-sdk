...
class DescribeSagManagementPortResponseBody(DaraModel):
    def __init__(
        self,
        gateway: str = None,
        ip: str = None,
        mask: str = None,
        request_id: str = None,
        task_states: List[main_models.DescribeSagManagementPortResponseBodyTaskStates] = None,
    ):
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('TaskStates') is not None:
            for k1 in m.get('TaskStates'):
                temp_model = main_models.DescribeSagManagementPortResponseBodyTaskStates()
                self.task_states.append(temp_model.from_map(k1))

        return self

class DescribeSagManagementPortResponseBodyTaskStates(DaraModel):
    def __init__(
        self,
