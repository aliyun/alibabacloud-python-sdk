...
class DescribeSagUserDnsResponseBody(DaraModel):
    def __init__(
        self,
        master_dns: str = None,
        request_id: str = None,
        slave_dns: str = None,
        task_states: List[main_models.DescribeSagUserDnsResponseBodyTaskStates] = None,
    ):
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('TaskStates') is not None:
            for k1 in m.get('TaskStates'):
                temp_model = main_models.DescribeSagUserDnsResponseBodyTaskStates()
                self.task_states.append(temp_model.from_map(k1))

        return self

class DescribeSagUserDnsResponseBodyTaskStates(DaraModel):
    def __init__(
        self,
