...
class DescribeSagGlobalRouteProtocolResponseBody(DaraModel):
    def __init__(
        self,
        request_id: str = None,
        route_protocol: str = None,
        task_states: List[main_models.DescribeSagGlobalRouteProtocolResponseBodyTaskStates] = None,
    ):
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('TaskStates') is not None:
            for k1 in m.get('TaskStates'):
                temp_model = main_models.DescribeSagGlobalRouteProtocolResponseBodyTaskStates()
                self.task_states.append(temp_model.from_map(k1))

        return self

class DescribeSagGlobalRouteProtocolResponseBodyTaskStates(DaraModel):
    def __init__(
        self,
