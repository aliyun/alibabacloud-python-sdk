...
class DescribeCustomerGatewaysRequest(DaraModel):
    def __init__(
        self,
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('Tag') is not None:
            for k1 in m.get('Tag'):
                temp_model = main_models.DescribeCustomerGatewaysRequestTag()
                self.tag.append(temp_model.from_map(k1))

        return self

class DescribeCustomerGatewaysRequestTag(DaraModel):
    def __init__(
        self,
        key: str = None,
        value: str = None,
