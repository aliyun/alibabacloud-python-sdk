...
class UnifiedSearchOutput(DaraModel):
    def __init__(
        self,
        cost_credits: main_models.UnifiedCostCredits = None,
        page_items: List[main_models.UnifiedPageItem] = None,
        query_context: main_models.UnifiedQueryContext = None,
        request_id: str = None,
        scene_items: List[main_models.UnifiedSceneItem] = None,
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('costCredits') is not None:
            temp_model = main_models.UnifiedCostCredits()
            self.cost_credits = temp_model.from_map(m.get('costCredits'))

        self.page_items = []
        if m.get('pageItems') is not None:
            for k1 in m.get('pageItems'):
                temp_model = main_models.UnifiedPageItem()
                self.page_items.append(temp_model.from_map(k1))

        if m.get('queryContext') is not None:
            temp_model = main_models.UnifiedQueryContext()
        ...
        if m.get('sceneItems') is not None:
            for k1 in m.get('sceneItems'):
                temp_model = main_models.UnifiedSceneItem()
