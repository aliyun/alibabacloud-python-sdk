...
class GlobalSearchResult(DaraModel):
    def __init__(
        self,
        page_items: List[main_models.GlobalPageItem] = None,
        query_context: main_models.GlobalQueryContext = None,
        request_id: str = None,
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('pageItems') is not None:
            for k1 in m.get('pageItems'):
                temp_model = main_models.GlobalPageItem()
                self.page_items.append(temp_model.from_map(k1))

        if m.get('queryContext') is not None:
            temp_model = main_models.GlobalQueryContext()
        ...
        if m.get('sceneItems') is not None:
            for k1 in m.get('sceneItems'):
                temp_model = main_models.GlobalSceneItem()
                self.scene_items.append(temp_model.from_map(k1))

        if m.get('searchInformation') is not None:
            temp_model = main_models.GlobalSearchInformation()
