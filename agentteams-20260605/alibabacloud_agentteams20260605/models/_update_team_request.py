...
class UpdateTeamRequest(DaraModel):
    def __init__(
        ...
        name: str = None,
        team_members: List[main_models.UpdateTeamRequestTeamMembers] = None,
    ):
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('TeamMembers') is not None:
            for k1 in m.get('TeamMembers'):
                temp_model = main_models.UpdateTeamRequestTeamMembers()
                self.team_members.append(temp_model.from_map(k1))

        return self

class UpdateTeamRequestTeamMembers(DaraModel):
    def __init__(
        self,
        name: str = None,
    ):
