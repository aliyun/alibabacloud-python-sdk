...
class DescribeAuditRecordsResponseBodyItems(DaraModel):
    def __init__(
        self,
        sqlrecord: List[main_models.DescribeAuditRecordsResponseBodyItemsSQLRecord] = None,
    ):
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('SQLRecord') is not None:
            for k1 in m.get('SQLRecord'):
                temp_model = main_models.DescribeAuditRecordsResponseBodyItemsSQLRecord()
                self.sqlrecord.append(temp_model.from_map(k1))

        return self

class DescribeAuditRecordsResponseBodyItemsSQLRecord(DaraModel):
    def __init__(
        self,
