...
class AiExtractTransformParameters(DaraModel):
    def __init__(
        self,
    ...
    def to_map(self):
        ...
        if self.extract_schema is not None:
            result['ExtractSchema'] = self.extract_schema
    ...
    def from_map(self, m: dict = None):
        m = m or dict()
        if m.get('ExtractSchema') is not None:
            self.extract_schema = m.get('ExtractSchema')
