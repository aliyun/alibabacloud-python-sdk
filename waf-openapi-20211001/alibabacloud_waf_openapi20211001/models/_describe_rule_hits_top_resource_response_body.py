...
class DescribeRuleHitsTopResourceResponseBody(DaraModel):
    def __init__(
        self,
        request_id: str = None,
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('RuleHitsTopResource') is not None:
            for k1 in m.get('RuleHitsTopResource'):
                temp_model = main_models.DescribeRuleHitsTopResourceResponseBodyRuleHitsTopResource()
                self.rule_hits_top_resource.append(temp_model.from_map(k1))

        return self

class DescribeRuleHitsTopResourceResponseBodyRuleHitsTopResource(DaraModel):
    def __init__(
        self,
        count: int = None,
        resource: str = None,
