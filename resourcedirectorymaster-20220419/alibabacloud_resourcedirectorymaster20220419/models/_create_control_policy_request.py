...
class CreateControlPolicyRequest(DaraModel):
    def __init__(
        ...
        tag: List[main_models.CreateControlPolicyRequestTag] = None,
    ):
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('Tag') is not None:
            for k1 in m.get('Tag'):
                temp_model = main_models.CreateControlPolicyRequestTag()
                self.tag.append(temp_model.from_map(k1))

        return self

class CreateControlPolicyRequestTag(DaraModel):
    def __init__(
        self,
        key: str = None,
        value: str = None,
