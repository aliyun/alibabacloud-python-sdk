# -*- coding: utf-8 -*-
# This file is auto-generated, don't edit it. Thanks.
from __future__ import annotations

from ._group_resources import GroupResources
from ._third_app import ThirdApp
from ._wa_id_permissions import WaIdPermissions
from ._batch_set_desktop_manager_request import BatchSetDesktopManagerRequest
from ._batch_set_desktop_manager_response_body import BatchSetDesktopManagerResponseBody
from ._batch_set_desktop_manager_response import BatchSetDesktopManagerResponse
from ._change_user_password_request import ChangeUserPasswordRequest
from ._change_user_password_response_body import ChangeUserPasswordResponseBody
from ._change_user_password_response import ChangeUserPasswordResponse
from ._check_used_property_request import CheckUsedPropertyRequest
from ._check_used_property_response_body import CheckUsedPropertyResponseBody
from ._check_used_property_response import CheckUsedPropertyResponse
from ._check_used_property_value_request import CheckUsedPropertyValueRequest
from ._check_used_property_value_response_body import CheckUsedPropertyValueResponseBody
from ._check_used_property_value_response import CheckUsedPropertyValueResponse
from ._create_group_request import CreateGroupRequest
from ._create_group_response_body import CreateGroupResponseBody
from ._create_group_response import CreateGroupResponse
from ._create_org_request import CreateOrgRequest
from ._create_org_response_body import CreateOrgResponseBody
from ._create_org_response import CreateOrgResponse
from ._create_property_request import CreatePropertyRequest
from ._create_property_response_body import CreatePropertyResponseBody
from ._create_property_response import CreatePropertyResponse
from ._create_resource_group_request import CreateResourceGroupRequest
from ._create_resource_group_response_body import CreateResourceGroupResponseBody
from ._create_resource_group_response import CreateResourceGroupResponse
from ._create_users_request import CreateUsersRequest
from ._create_users_response_body import CreateUsersResponseBody
from ._create_users_response import CreateUsersResponse
from ._delete_resource_group_request import DeleteResourceGroupRequest
from ._delete_resource_group_response_body import DeleteResourceGroupResponseBody
from ._delete_resource_group_response import DeleteResourceGroupResponse
from ._delete_user_property_value_request import DeleteUserPropertyValueRequest
from ._delete_user_property_value_response_body import DeleteUserPropertyValueResponseBody
from ._delete_user_property_value_response import DeleteUserPropertyValueResponse
from ._describe_group_user_request import DescribeGroupUserRequest
from ._describe_group_user_response_body import DescribeGroupUserResponseBody
from ._describe_group_user_response import DescribeGroupUserResponse
from ._describe_groups_request import DescribeGroupsRequest
from ._describe_groups_response_body import DescribeGroupsResponseBody
from ._describe_groups_response import DescribeGroupsResponse
from ._describe_mfa_devices_request import DescribeMfaDevicesRequest
from ._describe_mfa_devices_response_body import DescribeMfaDevicesResponseBody
from ._describe_mfa_devices_response import DescribeMfaDevicesResponse
from ._describe_org_by_layer_request import DescribeOrgByLayerRequest
from ._describe_org_by_layer_response_body import DescribeOrgByLayerResponseBody
from ._describe_org_by_layer_response import DescribeOrgByLayerResponse
from ._describe_orgs_request import DescribeOrgsRequest
from ._describe_orgs_shrink_request import DescribeOrgsShrinkRequest
from ._describe_orgs_response_body import DescribeOrgsResponseBody
from ._describe_orgs_response import DescribeOrgsResponse
from ._describe_resource_groups_request import DescribeResourceGroupsRequest
from ._describe_resource_groups_response_body import DescribeResourceGroupsResponseBody
from ._describe_resource_groups_response import DescribeResourceGroupsResponse
from ._describe_user_request import DescribeUserRequest
from ._describe_user_response_body import DescribeUserResponseBody
from ._describe_user_response import DescribeUserResponse
from ._describe_users_request import DescribeUsersRequest
from ._describe_users_shrink_request import DescribeUsersShrinkRequest
from ._describe_users_response_body import DescribeUsersResponseBody
from ._describe_users_response import DescribeUsersResponse
from ._filter_users_request import FilterUsersRequest
from ._filter_users_shrink_request import FilterUsersShrinkRequest
from ._filter_users_response_body import FilterUsersResponseBody
from ._filter_users_response import FilterUsersResponse
from ._get_manager_info_by_auth_code_request import GetManagerInfoByAuthCodeRequest
from ._get_manager_info_by_auth_code_response_body import GetManagerInfoByAuthCodeResponseBody
from ._get_manager_info_by_auth_code_response import GetManagerInfoByAuthCodeResponse
from ._init_tenant_alias_request import InitTenantAliasRequest
from ._init_tenant_alias_response_body import InitTenantAliasResponseBody
from ._init_tenant_alias_response import InitTenantAliasResponse
from ._list_property_request import ListPropertyRequest
from ._list_property_response_body import ListPropertyResponseBody
from ._list_property_response import ListPropertyResponse
from ._list_property_value_request import ListPropertyValueRequest
from ._list_property_value_response_body import ListPropertyValueResponseBody
from ._list_property_value_response import ListPropertyValueResponse
from ._lock_mfa_device_request import LockMfaDeviceRequest
from ._lock_mfa_device_response_body import LockMfaDeviceResponseBody
from ._lock_mfa_device_response import LockMfaDeviceResponse
from ._lock_users_request import LockUsersRequest
from ._lock_users_response_body import LockUsersResponseBody
from ._lock_users_response import LockUsersResponse
from ._modify_group_request import ModifyGroupRequest
from ._modify_group_response_body import ModifyGroupResponseBody
from ._modify_group_response import ModifyGroupResponse
from ._modify_org_request import ModifyOrgRequest
from ._modify_org_response_body import ModifyOrgResponseBody
from ._modify_org_response import ModifyOrgResponse
from ._modify_user_request import ModifyUserRequest
from ._modify_user_response_body import ModifyUserResponseBody
from ._modify_user_response import ModifyUserResponse
from ._move_org_request import MoveOrgRequest
from ._move_org_response_body import MoveOrgResponseBody
from ._move_org_response import MoveOrgResponse
from ._move_user_org_request import MoveUserOrgRequest
from ._move_user_org_response_body import MoveUserOrgResponseBody
from ._move_user_org_response import MoveUserOrgResponse
from ._query_sync_status_by_ali_uid_request import QuerySyncStatusByAliUidRequest
from ._query_sync_status_by_ali_uid_response_body import QuerySyncStatusByAliUidResponseBody
from ._query_sync_status_by_ali_uid_response import QuerySyncStatusByAliUidResponse
from ._remove_group_request import RemoveGroupRequest
from ._remove_group_response_body import RemoveGroupResponseBody
from ._remove_group_response import RemoveGroupResponse
from ._remove_mfa_device_request import RemoveMfaDeviceRequest
from ._remove_mfa_device_response_body import RemoveMfaDeviceResponseBody
from ._remove_mfa_device_response import RemoveMfaDeviceResponse
from ._remove_org_request import RemoveOrgRequest
from ._remove_org_response_body import RemoveOrgResponseBody
from ._remove_org_response import RemoveOrgResponse
from ._remove_property_request import RemovePropertyRequest
from ._remove_property_response_body import RemovePropertyResponseBody
from ._remove_property_response import RemovePropertyResponse
from ._remove_users_request import RemoveUsersRequest
from ._remove_users_response_body import RemoveUsersResponseBody
from ._remove_users_response import RemoveUsersResponse
from ._reset_user_password_request import ResetUserPasswordRequest
from ._reset_user_password_response_body import ResetUserPasswordResponseBody
from ._reset_user_password_response import ResetUserPasswordResponse
from ._set_user_property_value_request import SetUserPropertyValueRequest
from ._set_user_property_value_response_body import SetUserPropertyValueResponseBody
from ._set_user_property_value_response import SetUserPropertyValueResponse
from ._sync_all_edu_info_response_body import SyncAllEduInfoResponseBody
from ._sync_all_edu_info_response import SyncAllEduInfoResponse
from ._transfer_resources_into_group_request import TransferResourcesIntoGroupRequest
from ._transfer_resources_into_group_response_body import TransferResourcesIntoGroupResponseBody
from ._transfer_resources_into_group_response import TransferResourcesIntoGroupResponse
from ._transfer_resources_outof_group_request import TransferResourcesOutofGroupRequest
from ._transfer_resources_outof_group_response_body import TransferResourcesOutofGroupResponseBody
from ._transfer_resources_outof_group_response import TransferResourcesOutofGroupResponse
from ._unlock_mfa_device_request import UnlockMfaDeviceRequest
from ._unlock_mfa_device_response_body import UnlockMfaDeviceResponseBody
from ._unlock_mfa_device_response import UnlockMfaDeviceResponse
from ._unlock_users_request import UnlockUsersRequest
from ._unlock_users_response_body import UnlockUsersResponseBody
from ._unlock_users_response import UnlockUsersResponse
from ._update_property_request import UpdatePropertyRequest
from ._update_property_response_body import UpdatePropertyResponseBody
from ._update_property_response import UpdatePropertyResponse
from ._user_batch_join_group_request import UserBatchJoinGroupRequest
from ._user_batch_join_group_response_body import UserBatchJoinGroupResponseBody
from ._user_batch_join_group_response import UserBatchJoinGroupResponse
from ._user_batch_quit_group_request import UserBatchQuitGroupRequest
from ._user_batch_quit_group_response_body import UserBatchQuitGroupResponseBody
from ._user_batch_quit_group_response import UserBatchQuitGroupResponse
from ._third_app import ThirdAppOidcSsoConfigEndpoints
from ._third_app import ThirdAppOidcSsoConfig
from ._third_app import ThirdAppSecrets
from ._create_property_response_body import CreatePropertyResponseBodyCreateResultSavePropertyValueModelFailedPropertyValues
from ._create_property_response_body import CreatePropertyResponseBodyCreateResultSavePropertyValueModelSavePropertyValues
from ._create_property_response_body import CreatePropertyResponseBodyCreateResultSavePropertyValueModel
from ._create_property_response_body import CreatePropertyResponseBodyCreateResult
from ._create_users_request import CreateUsersRequestUsers
from ._create_users_response_body import CreateUsersResponseBodyCreateResultCreatedUsers
from ._create_users_response_body import CreateUsersResponseBodyCreateResultFailedUsers
from ._create_users_response_body import CreateUsersResponseBodyCreateResult
from ._describe_group_user_response_body import DescribeGroupUserResponseBodyGroups
from ._describe_group_user_response_body import DescribeGroupUserResponseBodyUsers
from ._describe_groups_response_body import DescribeGroupsResponseBodyGroupsAttachedLoginPolicy
from ._describe_groups_response_body import DescribeGroupsResponseBodyGroups
from ._describe_mfa_devices_response_body import DescribeMfaDevicesResponseBodyMfaDevicesAdUser
from ._describe_mfa_devices_response_body import DescribeMfaDevicesResponseBodyMfaDevices
from ._describe_org_by_layer_response_body import DescribeOrgByLayerResponseBodyOrgs
from ._describe_orgs_response_body import DescribeOrgsResponseBodyOrgsResourcePolicyList
from ._describe_orgs_response_body import DescribeOrgsResponseBodyOrgs
from ._describe_resource_groups_response_body import DescribeResourceGroupsResponseBodyResourceGroupAppRules
from ._describe_resource_groups_response_body import DescribeResourceGroupsResponseBodyResourceGroupPolicies
from ._describe_resource_groups_response_body import DescribeResourceGroupsResponseBodyResourceGroupTimers
from ._describe_resource_groups_response_body import DescribeResourceGroupsResponseBodyResourceGroup
from ._describe_user_response_body import DescribeUserResponseBodyUserExternalInfo
from ._describe_user_response_body import DescribeUserResponseBodyUserProperties
from ._describe_user_response_body import DescribeUserResponseBodyUser
from ._describe_users_response_body import DescribeUsersResponseBodyUsersExtrasResourcePolicyList
from ._describe_users_response_body import DescribeUsersResponseBodyUsersExtras
from ._describe_users_response_body import DescribeUsersResponseBodyUsersGroups
from ._describe_users_response_body import DescribeUsersResponseBodyUsersOrgs
from ._describe_users_response_body import DescribeUsersResponseBodyUsersProperties
from ._describe_users_response_body import DescribeUsersResponseBodyUsers
from ._filter_users_request import FilterUsersRequestOrderParam
from ._filter_users_request import FilterUsersRequestPropertyFilterParam
from ._filter_users_request import FilterUsersRequestPropertyKeyValueFilterParam
from ._filter_users_shrink_request import FilterUsersShrinkRequestPropertyFilterParam
from ._filter_users_shrink_request import FilterUsersShrinkRequestPropertyKeyValueFilterParam
from ._filter_users_response_body import FilterUsersResponseBodyUsersExternalInfo
from ._filter_users_response_body import FilterUsersResponseBodyUsersGroups
from ._filter_users_response_body import FilterUsersResponseBodyUsersOrgList
from ._filter_users_response_body import FilterUsersResponseBodyUsersResourcePolicyList
from ._filter_users_response_body import FilterUsersResponseBodyUsersSupportLoginIdps
from ._filter_users_response_body import FilterUsersResponseBodyUsersUserSetPropertiesModelsPropertyValues
from ._filter_users_response_body import FilterUsersResponseBodyUsersUserSetPropertiesModels
from ._filter_users_response_body import FilterUsersResponseBodyUsers
from ._init_tenant_alias_response_body import InitTenantAliasResponseBodyAliasInfo
from ._list_property_response_body import ListPropertyResponseBodyPropertiesPropertyValues
from ._list_property_response_body import ListPropertyResponseBodyProperties
from ._list_property_value_response_body import ListPropertyValueResponseBodyPropertyValueInfos
from ._lock_users_response_body import LockUsersResponseBodyLockUsersResultFailedUsers
from ._lock_users_response_body import LockUsersResponseBodyLockUsersResult
from ._query_sync_status_by_ali_uid_response_body import QuerySyncStatusByAliUidResponseBodyData
from ._remove_users_response_body import RemoveUsersResponseBodyRemoveUsersResultFailedUsers
from ._remove_users_response_body import RemoveUsersResponseBodyRemoveUsersResult
from ._reset_user_password_response_body import ResetUserPasswordResponseBodyResetUsersResultFailedUsers
from ._reset_user_password_response_body import ResetUserPasswordResponseBodyResetUsersResult
from ._transfer_resources_into_group_request import TransferResourcesIntoGroupRequestResources
from ._transfer_resources_outof_group_request import TransferResourcesOutofGroupRequestResources
from ._unlock_users_response_body import UnlockUsersResponseBodyUnlockUsersResultFailedUsers
from ._unlock_users_response_body import UnlockUsersResponseBodyUnlockUsersResult
from ._update_property_request import UpdatePropertyRequestPropertyValues
from ._update_property_response_body import UpdatePropertyResponseBodyUpdateResultSavePropertyValueModelFailedPropertyValues
from ._update_property_response_body import UpdatePropertyResponseBodyUpdateResultSavePropertyValueModelSavePropertyValues
from ._update_property_response_body import UpdatePropertyResponseBodyUpdateResultSavePropertyValueModel
from ._update_property_response_body import UpdatePropertyResponseBodyUpdateResult

__all__ = [
    GroupResources,
    ThirdApp,
    WaIdPermissions,
    BatchSetDesktopManagerRequest,
    BatchSetDesktopManagerResponseBody,
    BatchSetDesktopManagerResponse,
    ChangeUserPasswordRequest,
    ChangeUserPasswordResponseBody,
    ChangeUserPasswordResponse,
    CheckUsedPropertyRequest,
    CheckUsedPropertyResponseBody,
    CheckUsedPropertyResponse,
    CheckUsedPropertyValueRequest,
    CheckUsedPropertyValueResponseBody,
    CheckUsedPropertyValueResponse,
    CreateGroupRequest,
    CreateGroupResponseBody,
    CreateGroupResponse,
    CreateOrgRequest,
    CreateOrgResponseBody,
    CreateOrgResponse,
    CreatePropertyRequest,
    CreatePropertyResponseBody,
    CreatePropertyResponse,
    CreateResourceGroupRequest,
    CreateResourceGroupResponseBody,
    CreateResourceGroupResponse,
    CreateUsersRequest,
    CreateUsersResponseBody,
    CreateUsersResponse,
    DeleteResourceGroupRequest,
    DeleteResourceGroupResponseBody,
    DeleteResourceGroupResponse,
    DeleteUserPropertyValueRequest,
    DeleteUserPropertyValueResponseBody,
    DeleteUserPropertyValueResponse,
    DescribeGroupUserRequest,
    DescribeGroupUserResponseBody,
    DescribeGroupUserResponse,
    DescribeGroupsRequest,
    DescribeGroupsResponseBody,
    DescribeGroupsResponse,
    DescribeMfaDevicesRequest,
    DescribeMfaDevicesResponseBody,
    DescribeMfaDevicesResponse,
    DescribeOrgByLayerRequest,
    DescribeOrgByLayerResponseBody,
    DescribeOrgByLayerResponse,
    DescribeOrgsRequest,
    DescribeOrgsShrinkRequest,
    DescribeOrgsResponseBody,
    DescribeOrgsResponse,
    DescribeResourceGroupsRequest,
    DescribeResourceGroupsResponseBody,
    DescribeResourceGroupsResponse,
    DescribeUserRequest,
    DescribeUserResponseBody,
    DescribeUserResponse,
    DescribeUsersRequest,
    DescribeUsersShrinkRequest,
    DescribeUsersResponseBody,
    DescribeUsersResponse,
    FilterUsersRequest,
    FilterUsersShrinkRequest,
    FilterUsersResponseBody,
    FilterUsersResponse,
    GetManagerInfoByAuthCodeRequest,
    GetManagerInfoByAuthCodeResponseBody,
    GetManagerInfoByAuthCodeResponse,
    InitTenantAliasRequest,
    InitTenantAliasResponseBody,
    InitTenantAliasResponse,
    ListPropertyRequest,
    ListPropertyResponseBody,
    ListPropertyResponse,
    ListPropertyValueRequest,
    ListPropertyValueResponseBody,
    ListPropertyValueResponse,
    LockMfaDeviceRequest,
    LockMfaDeviceResponseBody,
    LockMfaDeviceResponse,
    LockUsersRequest,
    LockUsersResponseBody,
    LockUsersResponse,
    ModifyGroupRequest,
    ModifyGroupResponseBody,
    ModifyGroupResponse,
    ModifyOrgRequest,
    ModifyOrgResponseBody,
    ModifyOrgResponse,
    ModifyUserRequest,
    ModifyUserResponseBody,
    ModifyUserResponse,
    MoveOrgRequest,
    MoveOrgResponseBody,
    MoveOrgResponse,
    MoveUserOrgRequest,
    MoveUserOrgResponseBody,
    MoveUserOrgResponse,
    QuerySyncStatusByAliUidRequest,
    QuerySyncStatusByAliUidResponseBody,
    QuerySyncStatusByAliUidResponse,
    RemoveGroupRequest,
    RemoveGroupResponseBody,
    RemoveGroupResponse,
    RemoveMfaDeviceRequest,
    RemoveMfaDeviceResponseBody,
    RemoveMfaDeviceResponse,
    RemoveOrgRequest,
    RemoveOrgResponseBody,
    RemoveOrgResponse,
    RemovePropertyRequest,
    RemovePropertyResponseBody,
    RemovePropertyResponse,
    RemoveUsersRequest,
    RemoveUsersResponseBody,
    RemoveUsersResponse,
    ResetUserPasswordRequest,
    ResetUserPasswordResponseBody,
    ResetUserPasswordResponse,
    SetUserPropertyValueRequest,
    SetUserPropertyValueResponseBody,
    SetUserPropertyValueResponse,
    SyncAllEduInfoResponseBody,
    SyncAllEduInfoResponse,
    TransferResourcesIntoGroupRequest,
    TransferResourcesIntoGroupResponseBody,
    TransferResourcesIntoGroupResponse,
    TransferResourcesOutofGroupRequest,
    TransferResourcesOutofGroupResponseBody,
    TransferResourcesOutofGroupResponse,
    UnlockMfaDeviceRequest,
    UnlockMfaDeviceResponseBody,
    UnlockMfaDeviceResponse,
    UnlockUsersRequest,
    UnlockUsersResponseBody,
    UnlockUsersResponse,
    UpdatePropertyRequest,
    UpdatePropertyResponseBody,
    UpdatePropertyResponse,
    UserBatchJoinGroupRequest,
    UserBatchJoinGroupResponseBody,
    UserBatchJoinGroupResponse,
    UserBatchQuitGroupRequest,
    UserBatchQuitGroupResponseBody,
    UserBatchQuitGroupResponse,
    ThirdAppOidcSsoConfigEndpoints,
    ThirdAppOidcSsoConfig,
    ThirdAppSecrets,
    CreatePropertyResponseBodyCreateResultSavePropertyValueModelFailedPropertyValues,
    CreatePropertyResponseBodyCreateResultSavePropertyValueModelSavePropertyValues,
    CreatePropertyResponseBodyCreateResultSavePropertyValueModel,
    CreatePropertyResponseBodyCreateResult,
    CreateUsersRequestUsers,
    CreateUsersResponseBodyCreateResultCreatedUsers,
    CreateUsersResponseBodyCreateResultFailedUsers,
    CreateUsersResponseBodyCreateResult,
    DescribeGroupUserResponseBodyGroups,
    DescribeGroupUserResponseBodyUsers,
    DescribeGroupsResponseBodyGroupsAttachedLoginPolicy,
    DescribeGroupsResponseBodyGroups,
    DescribeMfaDevicesResponseBodyMfaDevicesAdUser,
    DescribeMfaDevicesResponseBodyMfaDevices,
    DescribeOrgByLayerResponseBodyOrgs,
    DescribeOrgsResponseBodyOrgsResourcePolicyList,
    DescribeOrgsResponseBodyOrgs,
    DescribeResourceGroupsResponseBodyResourceGroupAppRules,
    DescribeResourceGroupsResponseBodyResourceGroupPolicies,
    DescribeResourceGroupsResponseBodyResourceGroupTimers,
    DescribeResourceGroupsResponseBodyResourceGroup,
    DescribeUserResponseBodyUserExternalInfo,
    DescribeUserResponseBodyUserProperties,
    DescribeUserResponseBodyUser,
    DescribeUsersResponseBodyUsersExtrasResourcePolicyList,
    DescribeUsersResponseBodyUsersExtras,
    DescribeUsersResponseBodyUsersGroups,
    DescribeUsersResponseBodyUsersOrgs,
    DescribeUsersResponseBodyUsersProperties,
    DescribeUsersResponseBodyUsers,
    FilterUsersRequestOrderParam,
    FilterUsersRequestPropertyFilterParam,
    FilterUsersRequestPropertyKeyValueFilterParam,
    FilterUsersShrinkRequestPropertyFilterParam,
    FilterUsersShrinkRequestPropertyKeyValueFilterParam,
    FilterUsersResponseBodyUsersExternalInfo,
    FilterUsersResponseBodyUsersGroups,
    FilterUsersResponseBodyUsersOrgList,
    FilterUsersResponseBodyUsersResourcePolicyList,
    FilterUsersResponseBodyUsersSupportLoginIdps,
    FilterUsersResponseBodyUsersUserSetPropertiesModelsPropertyValues,
    FilterUsersResponseBodyUsersUserSetPropertiesModels,
    FilterUsersResponseBodyUsers,
    InitTenantAliasResponseBodyAliasInfo,
    ListPropertyResponseBodyPropertiesPropertyValues,
    ListPropertyResponseBodyProperties,
    ListPropertyValueResponseBodyPropertyValueInfos,
    LockUsersResponseBodyLockUsersResultFailedUsers,
    LockUsersResponseBodyLockUsersResult,
    QuerySyncStatusByAliUidResponseBodyData,
    RemoveUsersResponseBodyRemoveUsersResultFailedUsers,
    RemoveUsersResponseBodyRemoveUsersResult,
    ResetUserPasswordResponseBodyResetUsersResultFailedUsers,
    ResetUserPasswordResponseBodyResetUsersResult,
    TransferResourcesIntoGroupRequestResources,
    TransferResourcesOutofGroupRequestResources,
    UnlockUsersResponseBodyUnlockUsersResultFailedUsers,
    UnlockUsersResponseBodyUnlockUsersResult,
    UpdatePropertyRequestPropertyValues,
    UpdatePropertyResponseBodyUpdateResultSavePropertyValueModelFailedPropertyValues,
    UpdatePropertyResponseBodyUpdateResultSavePropertyValueModelSavePropertyValues,
    UpdatePropertyResponseBodyUpdateResultSavePropertyValueModel,
    UpdatePropertyResponseBodyUpdateResult
]
