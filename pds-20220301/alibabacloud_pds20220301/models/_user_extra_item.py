...
class UserExtraItem(DaraModel):
    def __init__(
        self,
        account: List[main_models.AccountLinkInfo] = None,
        avatar: str = None,
        created_at: str = None,
        creator: str = None,
        default_drive: main_models.BaseDriveResponse = None,
    ...
    def to_map(self):
        ...
        if self.parent_group is not None:
            for k1 in self.parent_group:
                result['parent_group'].append(k1.to_map() if k1 else None)
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('account') is not None:
            for k1 in m.get('account'):
                temp_model = main_models.AccountLinkInfo()
        ...
        if m.get('default_drive') is not None:
            temp_model = main_models.BaseDriveResponse()
            self.default_drive = temp_model.from_map(m.get('default_drive'))
        ...
        if m.get('parent_group') is not None:
            for k1 in m.get('parent_group'):
                temp_model = main_models.BaseDriveResponse()
                self.parent_group.append(temp_model.from_map(k1))
