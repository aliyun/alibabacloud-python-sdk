...
class DescribeImageGroupedVulListResponseBody(DaraModel):
    def __init__(
        ...
        # The number of image vulnerabilities displayed on each page in a paging query. Default value: **20**.
...
class DescribeImageGroupedVulListResponseBodyGroupedVulItems(DaraModel):
    def __init__(
        ...
        # Indicates whether the vulnerability supports agentless fix. A value of true indicates that agentless fix is supported. A value of false indicates that agentless fix is not supported. If this field is not returned, no corresponding fix capability information is available.
    ...
    def to_map(self):
        ...
        if self.cve_score is not None:
            result['CveScore'] = self.cve_score
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('CveScore') is not None:
            self.cve_score = m.get('CveScore')
