...
class GetServiceSettingsResponseBody(DaraModel):
    def __init__(
        self,
        request_id: str = None,
        service_settings: List[main_models.GetServiceSettingsResponseBodyServiceSettings] = None,
    ):
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('ServiceSettings') is not None:
            for k1 in m.get('ServiceSettings'):
                temp_model = main_models.GetServiceSettingsResponseBodyServiceSettings()
                self.service_settings.append(temp_model.from_map(k1))

        return self

class GetServiceSettingsResponseBodyServiceSettings(DaraModel):
    def __init__(
        self,
