...
class UpdateServiceCronScalerRequest(DaraModel):
    def __init__(
        self,
        exclude_dates: List[str] = None,
        scale_jobs: List[main_models.UpdateServiceCronScalerRequestScaleJobs] = None,
    ):
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('ScaleJobs') is not None:
            for k1 in m.get('ScaleJobs'):
                temp_model = main_models.UpdateServiceCronScalerRequestScaleJobs()
                self.scale_jobs.append(temp_model.from_map(k1))

        return self

class UpdateServiceCronScalerRequestScaleJobs(DaraModel):
    def __init__(
        self,
        name: str = None,
        schedule: str = None,
