...
class CreateGrafanaWorkspaceRequest(DaraModel):
    def __init__(
        self,
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('Tags') is not None:
            for k1 in m.get('Tags'):
                temp_model = main_models.CreateGrafanaWorkspaceRequestTags()
                self.tags.append(temp_model.from_map(k1))

        return self

class CreateGrafanaWorkspaceRequestTags(DaraModel):
    def __init__(
        self,
        key: str = None,
        value: str = None,
