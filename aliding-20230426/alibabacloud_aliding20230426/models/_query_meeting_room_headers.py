...
class QueryMeetingRoomHeaders(DaraModel):
    def __init__(
        self,
        common_headers: Dict[str, str] = None,
        account_context: main_models.QueryMeetingRoomHeadersAccountContext = None,
    ):
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('AccountContext') is not None:
            temp_model = main_models.QueryMeetingRoomHeadersAccountContext()
            self.account_context = temp_model.from_map(m.get('AccountContext'))

        return self

class QueryMeetingRoomHeadersAccountContext(DaraModel):
    def __init__(
        self,
        account_id: str = None,
    ):
