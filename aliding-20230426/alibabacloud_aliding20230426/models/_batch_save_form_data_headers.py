...
class BatchSaveFormDataHeaders(DaraModel):
    def __init__(
        self,
        common_headers: Dict[str, str] = None,
        account_context: main_models.BatchSaveFormDataHeadersAccountContext = None,
    ):
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('AccountContext') is not None:
            temp_model = main_models.BatchSaveFormDataHeadersAccountContext()
            self.account_context = temp_model.from_map(m.get('AccountContext'))

        return self

class BatchSaveFormDataHeadersAccountContext(DaraModel):
    def __init__(
        self,
        account_id: str = None,
    ):
