...
class DescribePortViewSourceCountriesResponseBody(DaraModel):
    def __init__(
        self,
        request_id: str = None,
        source_countrys: List[main_models.DescribePortViewSourceCountriesResponseBodySourceCountrys] = None,
    ):
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('SourceCountrys') is not None:
            for k1 in m.get('SourceCountrys'):
                temp_model = main_models.DescribePortViewSourceCountriesResponseBodySourceCountrys()
                self.source_countrys.append(temp_model.from_map(k1))

        return self

class DescribePortViewSourceCountriesResponseBodySourceCountrys(DaraModel):
    def __init__(
        self,
        count: int = None,
        country_id: str = None,
