...
class ListOfficeSiteUsersResponseBody(DaraModel):
    def __init__(
        self,
        next_token: str = None,
        request_id: str = None,
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('Users') is not None:
            for k1 in m.get('Users'):
                temp_model = main_models.ListOfficeSiteUsersResponseBodyUsers()
                self.users.append(temp_model.from_map(k1))

        return self

class ListOfficeSiteUsersResponseBodyUsers(DaraModel):
    def __init__(
        self,
