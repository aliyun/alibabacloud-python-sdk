...
class QueryActiveUserStatisticResponseBody(DaraModel):
    def __init__(
        self,
        request_id: str = None,
        total_count: int = None,
        user_statistic_list: List[main_models.QueryActiveUserStatisticResponseBodyUserStatisticList] = None,
    ):
        # The request ID.
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('UserStatisticList') is not None:
            for k1 in m.get('UserStatisticList'):
                temp_model = main_models.QueryActiveUserStatisticResponseBodyUserStatisticList()
                self.user_statistic_list.append(temp_model.from_map(k1))

        return self

class QueryActiveUserStatisticResponseBodyUserStatisticList(DaraModel):
    def __init__(
        self,
