...
class PublicUpdateTemplateRegistryConfig(DaraModel):
    def __init__(
        self,
        auth_config: main_models.PublicUpdateTemplateRegistryAuthConfig = None,
        cert_config: main_models.PublicUpdateTemplateRegistryCertConfig = None,
        network_config: main_models.PublicUpdateTemplateRegistryNetworkConfig = None,
    ):
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('authConfig') is not None:
            temp_model = main_models.PublicUpdateTemplateRegistryAuthConfig()
            self.auth_config = temp_model.from_map(m.get('authConfig'))

        if m.get('certConfig') is not None:
            temp_model = main_models.PublicUpdateTemplateRegistryCertConfig()
            self.cert_config = temp_model.from_map(m.get('certConfig'))

        if m.get('networkConfig') is not None:
            temp_model = main_models.PublicUpdateTemplateRegistryNetworkConfig()
