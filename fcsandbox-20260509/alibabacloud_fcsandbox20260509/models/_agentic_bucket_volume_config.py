# -*- coding: utf-8 -*-
# This file is auto-generated, don't edit it. Thanks.
from __future__ import annotations

from darabonba.model import DaraModel

class AgenticBucketVolumeConfig(DaraModel):
    def __init__(
        self,
        agentic_bucket: str = None,
        bucket_name: str = None,
        bucket_path: str = None,
        endpoint: str = None,
        read_only: bool = None,
    ):
        # AgenticBucket
        self.agentic_bucket = agentic_bucket
        # The BucketSpace of the agentic child.
        self.bucket_name = bucket_name
        # The file path under the BucketSpace.
        self.bucket_path = bucket_path
        # Agentic Bucket Endpoint
        self.endpoint = endpoint
        # Indicates whether the bucket is read-only.
        self.read_only = read_only

    def validate(self):
        pass

    def to_map(self):
        result = dict()
        _map = super().to_map()
        if _map is not None:
            result = _map
        if self.agentic_bucket is not None:
            result['agenticBucket'] = self.agentic_bucket

        if self.bucket_name is not None:
            result['bucketName'] = self.bucket_name

        if self.bucket_path is not None:
            result['bucketPath'] = self.bucket_path

        if self.endpoint is not None:
            result['endpoint'] = self.endpoint

        if self.read_only is not None:
            result['readOnly'] = self.read_only

        return result

    def from_map(self, m: dict = None):
        m = m or dict()
        if m.get('agenticBucket') is not None:
            self.agentic_bucket = m.get('agenticBucket')

        if m.get('bucketName') is not None:
            self.bucket_name = m.get('bucketName')

        if m.get('bucketPath') is not None:
            self.bucket_path = m.get('bucketPath')

        if m.get('endpoint') is not None:
            self.endpoint = m.get('endpoint')

        if m.get('readOnly') is not None:
            self.read_only = m.get('readOnly')

        return self

