...
class TransitVisaRequest(DaraModel):
    def __init__(
        self,
        flight_segment_param_list: List[main_models.TransitVisaRequestFlightSegmentParamList] = None,
    ):
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('flight_segment_param_list') is not None:
            for k1 in m.get('flight_segment_param_list'):
                temp_model = main_models.TransitVisaRequestFlightSegmentParamList()
                self.flight_segment_param_list.append(temp_model.from_map(k1))

        return self

class TransitVisaRequestFlightSegmentParamList(DaraModel):
    def __init__(
        self,
