...
class CreateLayer4RuleRequest(DaraModel):
    def __init__(
        self,
        listeners: str = None,
        proxy_enable: int = None,
        us_timeout: main_models.CreateLayer4RuleRequestUsTimeout = None,
    ):
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('UsTimeout') is not None:
            temp_model = main_models.CreateLayer4RuleRequestUsTimeout()
            self.us_timeout = temp_model.from_map(m.get('UsTimeout'))

        return self

class CreateLayer4RuleRequestUsTimeout(DaraModel):
    def __init__(
        self,
