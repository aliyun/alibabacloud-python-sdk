...
class UpdateKibanaWhiteIpsRequest(DaraModel):
    def __init__(
        ...
        # The IP access whitelist for the Kibana node. Cannot be set together with the whitelist group.
        self.kibana_ipwhitelist = kibana_ipwhitelist
        # Updates the Kibana access whitelist configuration by whitelist group. Only one whitelist group can be updated at a time.
        # 
        # You cannot configure kibanaIPWhitelist and whiteIpGroup at the same time.
        self.white_ip_group = white_ip_group
        # A unique token generated by the client to ensure the idempotence of the request. The value must be unique across different requests and can be up to 64 ASCII characters in length.
...
class UpdateKibanaWhiteIpsRequestWhiteIpGroup(DaraModel):
    def __init__(
        ...
        self.ips = ips
        # The whitelist type. Only PUBLIC_KIBANA (public access whitelist) is supported.
