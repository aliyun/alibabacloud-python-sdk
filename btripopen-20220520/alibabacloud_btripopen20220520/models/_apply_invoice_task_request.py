...
class ApplyInvoiceTaskRequest(DaraModel):
    def __init__(
        self,
        bill_date: str = None,
        invoice_task_list: List[main_models.ApplyInvoiceTaskRequestInvoiceTaskList] = None,
    ):
    ...
    def validate(self):
        if self.invoice_task_list:
            for v1 in self.invoice_task_list:
                 if v1:
                    v1.validate()
    ...
    def to_map(self):
        ...
        if self.invoice_task_list is not None:
            for k1 in self.invoice_task_list:
                result['invoice_task_list'].append(k1.to_map() if k1 else None)
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('invoice_task_list') is not None:
            for k1 in m.get('invoice_task_list'):
                temp_model = main_models.ApplyInvoiceTaskRequestInvoiceTaskList()
                self.invoice_task_list.append(temp_model.from_map(k1))

        return self

class ApplyInvoiceTaskRequestInvoiceTaskList(DaraModel):
    def __init__(
        self,
        contact: str = None,
        email: str = None,
