...
class ListQueryProcessorsResponseBody(DaraModel):
    def __init__(
        self,
        request_id: str = None,
        result: List[main_models.ListQueryProcessorsResponseBodyResult] = None,
    ):
        # The request ID.
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('result') is not None:
            for k1 in m.get('result'):
                temp_model = main_models.ListQueryProcessorsResponseBodyResult()
                self.result.append(temp_model.from_map(k1))

        return self

class ListQueryProcessorsResponseBodyResult(DaraModel):
    def __init__(
        self,
        active: bool = None,
        created: int = None,
