...
class BatchEventRebootInstanceRequest(DaraModel):
    def __init__(
        self,
        event_infos: List[main_models.BatchEventRebootInstanceRequestEventInfos] = None,
    ):
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('EventInfos') is not None:
            for k1 in m.get('EventInfos'):
                temp_model = main_models.BatchEventRebootInstanceRequestEventInfos()
                self.event_infos.append(temp_model.from_map(k1))

        return self

class BatchEventRebootInstanceRequestEventInfos(DaraModel):
    def __init__(
        self,
        event_id: str = None,
        ops_type: str = None,
