...
class DetachHostAccountsFromHostShareKeyResponseBody(DaraModel):
    def __init__(
        self,
        request_id: str = None,
        results: List[main_models.DetachHostAccountsFromHostShareKeyResponseBodyResults] = None,
    ):
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('Results') is not None:
            for k1 in m.get('Results'):
                temp_model = main_models.DetachHostAccountsFromHostShareKeyResponseBodyResults()
                self.results.append(temp_model.from_map(k1))

        return self

class DetachHostAccountsFromHostShareKeyResponseBodyResults(DaraModel):
    def __init__(
        self,
        code: str = None,
