...
class ListUsersResponseBodyData(DaraModel):
    def __init__(
        self,
        user: List[main_models.ListUsersResponseBodyDataUser] = None,
    ):
        self.user = user
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('User') is not None:
            for k1 in m.get('User'):
                temp_model = main_models.ListUsersResponseBodyDataUser()
                self.user.append(temp_model.from_map(k1))

        return self

class ListUsersResponseBodyDataUser(DaraModel):
    def __init__(
        self,
        ali_uid: str = None,
