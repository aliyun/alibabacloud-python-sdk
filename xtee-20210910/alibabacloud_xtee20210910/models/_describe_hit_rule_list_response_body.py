...
class DescribeHitRuleListResponseBody(DaraModel):
    def __init__(
        self,
        request_id: str = None,
    ...
    def from_map(self, m: dict = None):
        ...
        if m.get('resultObject') is not None:
            for k1 in m.get('resultObject'):
                temp_model = main_models.DescribeHitRuleListResponseBodyResultObject()
                self.result_object.append(temp_model.from_map(k1))

        return self

class DescribeHitRuleListResponseBodyResultObject(DaraModel):
    def __init__(
        self,
        hit_count: int = None,
        rule_name: str = None,
