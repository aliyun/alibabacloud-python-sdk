...
class Client(RPCClient):
    ...
    def classify_fnf(self, request, runtime):
        UtilClient.validate_model(request)
        return imageprocess_20200320_models.ClassifyFNFResponse().from_map(self.do_request("ClassifyFNF", "HTTPS", "POST", "2020-03-20", "AK", None, request.to_map(), runtime))
    ...
    def run_ctregistration(self, request, runtime):
        UtilClient.validate_model(request)
        return imageprocess_20200320_models.RunCTRegistrationResponse().from_map(self.do_request("RunCTRegistration", "HTTPS", "POST", "2020-03-20", "AK", None, request.to_map(), runtime))

    def detect_hip_keypoint_xray(self, request, runtime):
        UtilClient.validate_model(request)
        return imageprocess_20200320_models.DetectHipKeypointXRayResponse().from_map(self.do_request("DetectHipKeypointXRay", "HTTPS", "POST", "2020-03-20", "AK", None, request.to_map(), runtime))
    ...
    def calc_cacs(self, request, runtime):
        UtilClient.validate_model(request)
        return imageprocess_20200320_models.CalcCACSResponse().from_map(self.do_request("CalcCACS", "HTTPS", "POST", "2020-03-20", "AK", None, request.to_map(), runtime))
